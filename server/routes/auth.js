...
router.post('/forgot-password', authLimiter, async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO password_resets (id, user_id, email, otp, otp_attempts, expires_at)
       VALUES ($1, $2, $3, $4, 0, $5)
       ON CONFLICT (email) DO UPDATE SET
         otp = EXCLUDED.otp, otp_attempts = 0, expires_at = EXCLUDED.expires_at`,
      [crypto.randomUUID(), user.id, email, otp, expiresAt]
    );
...
router.post('/reset-password', otpLimiter, async (req, res) => {
  ...
  try {
    ...
    if (reset.otp !== String(otp).trim()) {
      await pool.query('UPDATE password_resets SET otp_attempts = otp_attempts + 1 WHERE email = $1', [email]);
